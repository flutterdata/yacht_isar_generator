...
class YachtIsarAnalyzer {
  ObjectInfo analyzeCollection(Element element) {
    ...
    for (final propertyElement in modelClass.yachtAllAccessors) {
  ...
  ObjectInfo analyzeEmbedded(Element element) {
    ...
    final hasIndex = modelClass.yachtAllAccessors.any(
  ...
  Map<String, String> _getEmbeddedDartNames(ClassElement element) {
    void _fillNames(Map<String, String> names, ClassElement element) {
      for (final property in element.yachtAllAccessors) {
...
extension ClassElementX on ClassElement {
  List<PropertyInducingElement> get yachtAllAccessors {
    final ignoreFields = embeddedAnnotation?.ignore ?? {};
    return [
      ...accessors.mapNotNull((e) => e.variable),
      for (InterfaceType supertype in allSupertypes) ...[
        // TODO this DataModel check brittle AF
        if (!supertype.isDartCoreObject &&
            supertype.element.displayName.startsWith('DataModel'))
